...
def date_to_month_pandas(d):
    # You may need to modify this function, depending on your data types.
    return pd.Series.apply(d, date_to_month)
...
def pivot_months_pandas(data):
    ...
    date = data.ix[:,'date']
    data['month'] = date_to_month_pandas(date)
    grouped_data = data.groupby(['name', 'month'])
    totals = grouped_data.aggregate('sum').reset_index()
    totals = totals.pivot(index='name', columns='month', values='precipitation')
    counts = grouped_data.aggregate('count').reset_index()
